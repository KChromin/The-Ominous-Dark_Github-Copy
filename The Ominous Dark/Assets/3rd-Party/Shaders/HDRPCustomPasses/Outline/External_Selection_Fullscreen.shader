Shader "External/Selection/Fullscreen"
{
    properties
    {
        ...
        //  _BehindFactor("Behind Factor", Range(0,1)) = 0
    }
    ...
    float4 FullScreenPass(Varyings varyings) : SV_Target
    {
        ...
        {
            outline = max(SampleCustomColor(posInput.positionNDC + uvOffsetPerPixel * _OutlineWidth * offsets[i]),
  outline);
        }
        ...
        // float4 innerColor = SAMPLE_TEXTURE2D(_Texture, s_trilinear_repeat_sampler, posInput.positionSS / _TextureSize) *
        //  _InnerColor;

        //  float4 innerColor = SAMPLE_TEXTURE2D(_Texture, s_trilinear_repeat_sampler, posInput.tileCoord) *
        //   _InnerColor;
