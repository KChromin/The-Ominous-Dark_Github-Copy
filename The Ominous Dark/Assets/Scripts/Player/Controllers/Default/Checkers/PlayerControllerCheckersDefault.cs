...
namespace NOS.Player.Controller.Default
{
    public class PlayerControllerCheckersDefault : ControllerBase
    {
        ...
        private void SlopeCheck()
        {
            ...
            {
                Vector3 origin = _valuesDefault.GroundCheckHit.point;

                if (Physics.Raycast(origin + _parameters.slopeCheckRaycastOffset, Vector3.down, out RaycastHit hit, _parameters.slopeCheckRaycastOffset.y + 0.01f, _parameters.groundLayer, QueryTriggerInteraction.Ignore))
                {
                    _valuesDefault.SlopeCheckNormal = hit.normal;
                    _conditions.cases.isOnTooSteepSlope = Vector3.Angle(hit.normal, Vector3.up) > _parameters.slopeCheckMaxSlope;
                }
                else
                {
